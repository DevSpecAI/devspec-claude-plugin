...
| **Commit observation** (`commit-observation.mjs`, item 27fab61a) | `PreToolUse` sees the command, `PostToolUse` sees its output — but `git commit -q` prints nothing | Remembers HEAD before a commit-producing command and reports the sha it created via `report_commit_provenance`, for EVERY commit regardless of reference or shape. Parses `[branch shortsha]` first, falls back to a HEAD before/after comparison in the directory the command actually commits in. Never denies, never rewrites, swallows every failure. Applies the same positive jurisdiction marker test as the gate, in both hooks, so commits in repositories this project has no claim over are never reported. Binds to the connection by conversation id only — never by agent name, since a wrong guess attributes someone else's commit | Server-side analyser still reconciles the commit, without agent attribution |
