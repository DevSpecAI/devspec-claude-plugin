...
allowed-tools: Read, Write, Edit, Bash, Grep, Glob, Agent, mcp__devspec__list_projects, mcp__devspec__get_action_items, mcp__devspec__get_next_work_item, mcp__devspec__claim_work_item, mcp__devspec__update_action_item, mcp__devspec__spin_off_action_item, mcp__devspec__record_implementation, mcp__devspec__get_project_summary, mcp__devspec__add_commit_reference, mcp__devspec__add_implementation_note, mcp__devspec__send_heartbeat, mcp__devspec__get_action_item_siblings, mcp__devspec__get_session_transcript, mcp__devspec__get_memory, mcp__devspec__search_memories, mcp__devspec__get_decisions, mcp__devspec__get_conventions, mcp__devspec__get_resources, mcp__devspec__search_resources, mcp__devspec__get_resource, mcp__devspec__record_memory, mcp__devspec__supersede_memory, mcp__devspec__retract_memory, mcp__devspec__create_resource, mcp__devspec__update_resource, mcp__devspec__supersede_resource, mcp__devspec__archive_resource, mcp__devspec__get_assignment, mcp__devspec__acknowledge_assignment, mcp__devspec__resolve_assignment, mcp__devspec__resolve_action_item_conflict, mcp__devspec__link_action_items, mcp__devspec__unlink_action_items, mcp__devspec__record_criterion_verdicts, mcp__devspec__classify_criterion
---
...
- **Never sign off — but DO record what you observed.** Two different acts, and the split is the whole point. **Never verify:** `verify_action_item` is a present human's decision and is not available to this skill. You are unattended, so there is nobody here to confirm the work. **But do record observations:** if an acceptance criterion can be checked by running or querying something (a migration applied, a column populated, a log field present, an endpoint's response shape), check it and record it with `record_criterion_verdicts` — that is reporting what you saw, not signing anything off, and the server decides on its own whether the criteria add up. Evidence must name what you RAN or QUERIED and what CAME BACK; reading code is inference and is rejected. Anything a person has to judge (taste, "does this look right") gets `classify_criterion` with a reason — never a tick. Keep that apart from verdict `unknown`, which means "nobody has checked this YET" and stays retryable by anything better equipped: marking a line as needing a person merely because YOU could not check it, unattended, dumps mechanical work on a human, and that is the failure the two states exist to separate.
...
1. **CLAIM**: Call `claim_work_item` with `action_item_id` and `agent_branch: <branch_name>`. Branch name format: `{branch_prefix}{item_id_first_8_chars}`. This is an atomic transition (staged → in_progress) — if the item is no longer staged (another agent claimed it), the call fails. On failure, skip to the next cycle.
   ...
   **Memory context (MANDATORY — never skip):** Before any file reading or implementation, call `search_memories({ project_id, query: "<action item title>" })` (project-scoped — pass the project resolved at startup) to retrieve related architecture decisions, coding conventions, known risks, and team preferences. Run it **in parallel** with the `get_action_item_siblings` call above when the item belongs to a brief. The search returns CARDS — title, one-line summary, id — not full text. Treat the returned memories as **hard constraints**, and `get_memory` any card that looks like it binds this work before you rely on it. A summary states the decision; the body carries the qualifications and exceptions, and a constraint obeyed without its exceptions is how you confidently do the wrong thing. Do not run the loop blind to recorded knowledge.
