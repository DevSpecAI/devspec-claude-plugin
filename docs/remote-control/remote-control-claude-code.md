...
**An unlinked folder waits instead (item fa9b809b).** When the only answer is that the folder names no project (`folder_not_linked`), or the server declined the project it named (`register_refused`), the listener recomputes `folderLinkFingerprint` (`devspec-scope.mjs`) every `LINK_WATCH_MS` (3 s). That is stat calls on the pin files and the git config that holds `origin`, never a git process or the network. Only when the fingerprint changes does it run the real lookup, and when the folder now names a project it connects exactly as at startup. A `git init` with no remote, or a half-written pin, just becomes the new baseline. A folder that never gets a pin or a remote never changes its fingerprint, so it never reaches the server.

**A pinned folder that gets a repository (`repo-link-nudge.mjs`, PostToolUse on Bash).** After a command that looks like it created or re-pointed `origin` (`git remote add|set-url`, `gh repo create`), in a folder with a pin: if `list_projects({ git_remote })` says the remote does not resolve to the pinned project, the hook adds one line of context telling the model to ask the person to add the repository under the project's **Settings → Integrations → Repositories**. It says this once per conversation, project and remote, and stays silent on any failure.
