...
allowed-tools: Read, Grep, Glob, Bash, Agent, mcp__devspec__get_session_transcript, mcp__devspec__post_session_message, mcp__devspec__get_memory, mcp__devspec__search_memories, mcp__devspec__record_memory, mcp__devspec__supersede_memory, mcp__devspec__retract_memory, mcp__devspec__create_action_item, mcp__devspec__update_action_item
---
...
5. **Branch on `mode`:**
   ...
   - **Durable knowledge.** If the discussion settled something durable about the *project* — a decision, convention, architecture fact, or risk that outlives this session — persist it with `record_memory` (`search_memories` FIRST — it returns a CARD (title, one-line summary, id), so `get_memory` the closest match and read it in full before acting on it; then `supersede_memory`/`retract_memory` the stale match instead of duplicating; shared knowledge only, not transient or obvious details — don't record aggressively). DevSpec memory is the team's **shared** source of truth, distinct from your own local memory (Claude Code's `CLAUDE.md` / built-in notes) where personal or machine-specific notes belong — keeping that boundary is what stops DevSpec going stale.
   - Then output a local confirmation and stop (see Output).
