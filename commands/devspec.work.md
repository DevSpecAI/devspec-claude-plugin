...
**6b. More than one id? Reserve the set before you start the first.** The app emits one command carrying several ids (`/devspec.work id1,id2,id3`) precisely so one agent works them in order. Call `reserve_work_items({ action_item_ids: [<ids in the order given>], connection_id })` once, up front — that is what stops a second agent picking up id3 while you are still on id1. Then run steps 7–15 per item, claiming each as you reach it.

Read `skipped` in the response and SAY what it says: an item another agent already holds comes back with a reason naming the holder, not an error. Work the rest; do not report the whole batch as yours. There is nothing to resolve at the end — the reservation closes itself when its last member is recorded, failed or released.
