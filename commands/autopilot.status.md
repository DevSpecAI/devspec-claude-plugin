...
1. Call `get_project_summary({ project_id })` for settings, `get_action_items` with `project_id, agent_ready: true, agent_activity: 'staged'` for staged count, and `get_action_items` with `project_id, agent_activity: 'in_progress'` for active count — **all in parallel**
2. Output:
