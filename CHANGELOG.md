...
- **Dual-host with Grok Build:** documented that this plugin and [DevSpec for Grok Build](https://github.com/DevSpecAI/DevSpec-Grok-Build-Plugin) can both stay installed for the same projects. Separate install paths (`~/.claude/plugins` vs `~/.grok/installed-plugins`); both packages are named `devspec` so Grok shadows Claude’s package inside Grok and slash menus never mix. Do not layer flat skill copies under `.agents/skills` on top of either plugin.
- **Token docs — one account-wide token, reusable everywhere:** the README now points token creation at **You → Connections → Connect a tool** (was the stale "Settings → API"), corrects the storage note (macOS Keychain vs an encrypted credentials file on Linux/Windows), explains the plugin bundles the MCP server so there's nothing to add to `.mcp.json`, and documents entering/changing the token via `/plugin` → **Installed** → **DevSpec**. Reflects the new model: one account-wide token, reused in every tool and revealable again any time from You → Connections.
